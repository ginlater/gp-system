...
private fun PreviewAudio(
    ...
    // 把播放态上报给屏级（自动轮询「播放中跳过」）。离屏时同时置回 false。
    ...
    val canPreview = !processing && !loadingUrl
    // 播放器就绪前用 --:--（durationLabel 是「18分23秒」式人读串，不塞进 mm:ss 槽位）
    val durLabel = durationMs.takeIf { it > 0 }?.let { msToLabel(it) } ?: "--:--"
    ...
    AudioPreviewBar(
        ...
        positionLabel = msToLabel(positionMs),
...
private fun subLabel(rec: PendingRecording, crossDay: Boolean): String {
    ...
    return "服务日期 $date$crossTag · $dur$speakers"
}
