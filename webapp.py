...
APP_LATEST_VERSION_CODE = 24
APP_LATEST_VERSION_NAME = "2.1.13"
APP_MIN_VERSION_CODE = 24                # 低于此值的客户端 → 强制更新（v24：①陪伴页顶部显示版本号(修 appVersion 桥原本写死返 2.0.0)；②补传队列不再被"传不动的僵尸段"退避周期堵死后面就绪任务(kickWorker 跳过冷却队头挑 ready)；③「上传中」可取消(cancelPenUpload 桥真正从队列移除任务)。承接 v23 全部。真机验证损坏段一键重传替换成功）
APP_UPDATE_NOTE = "本次更新：① 陪伴页顶部可看到当前版本号，方便确认是否已是最新；② 上传更顺畅——有一段没传上来时不再拖慢其他段；③「上传中」现在可以取消了；④ 遇到上传异常可一键重新上传，不用再手动处理。更新后更稳更省心 💛"
