...
APP_LATEST_VERSION_CODE = 23
APP_LATEST_VERSION_NAME = "2.1.12"
APP_MIN_VERSION_CODE = 23                # 低于此值的客户端 → 强制更新（v23：连接级前台保活——笔连着就挂前台服务托底，App 切后台/锁屏不再被系统断蓝牙(录音级仍另持唤醒锁)；电池白名单引导弹框全员化(原来只有手机麦路径才弹)。真机(vivo V2445A)已验证锁屏进程存活+保活·连接级↑）
APP_UPDATE_NOTE = "本次更新：① 录音笔连接更稳——App 退到后台或锁屏也保持连接，不再容易断开；② 首次启动会请求「允许后台运行」，请点允许，保证陪伴过程不中断。小提醒：陪伴进行中请不要在最近任务里划掉 App 哦 💛"
